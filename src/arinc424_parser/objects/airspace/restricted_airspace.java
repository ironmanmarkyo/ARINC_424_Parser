 ...
 * @author yohan
 */
public class restricted_airspace {
...
//    String EntryReport;
    ...
    public restricted_airspace(String temp){
        ...
        this.RestrictiveType = temp.substring(8,9).trim();
        this.RestrictiveAirspaceDesignation = temp.substring(9,19).trim();
...
//        this.EntryReport = temp.substring(30,31).trim();
        ...
        this.RestrictiveAirspaceName = temp.substring(93,123).trim();
        this.RestrictiveAirspaceName = this.RestrictiveAirspaceName.replace("\'", " ");
...
//        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    ...
    public void InsertDatabase(Connection a){
        ...
        try {
            ...
            String ins = "INSERT INTO restricted_airspace VALUES(" +
                "\'"+RecordType+"\'" +","+
                "\'"+CustomerAreaCode+"\'" +","+
                "\'"+SectionCode+"\'" +","+
                "\'"+SubsectionCode+"\'"+","+
...
//                System.out.println(ins);
    ...
    public static void CalculateGeolocations_arc(Connection a){
        try {
            ...
            String sql = "Select exists(select from information_schema.columns where table_name = 'restricted_airspace' and column_name = 'geolocation_arc');";
...
//                System.out.println("postgis not there");
                String cre = "alter table restricted_airspace add column geolocation_arc geography(point);";
                stmt.execute(cre);
            }
            String up = "update restricted_airspace set geolocation_arc = ST_MakePoint(arc_longitude, arc_latitude);";
...
//            System.out.println(latitude);
    ...
    public static void CalculateGeolocations(Connection a){
        try {
            Statement stmt = a.createStatement();
            String sql = "Select exists(select from information_schema.columns where table_name = 'restricted_airspace' and column_name = 'geolocation');";
            ResultSet rs = stmt.executeQuery(sql);
            rs.next();
            String result = rs.getString("exists");
            rs.close();
            if (!result.equalsIgnoreCase("t")) {
//                System.out.println("postgis not there");
                String cre = "alter table restricted_airspace add column geolocation geography(point);";
                stmt.execute(cre);
            }
            String up = "update restricted_airspace set geolocation = ST_MakePoint(longitude, latitude);";
            stmt.executeUpdate(up);
            stmt.close();
            a.commit();
//            a.close();
